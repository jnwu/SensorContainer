	...
	$.fn.thing = function(params) {
		...
		function getEvents(params, obj) {
			...
			$.ajax({
				type: "GET",
				crossDomain: true,
				url: params.url+"/things/"+params.thingIdUnique+"/events?limit=1&waitTime=30&after="+params.timestamp,
				dataType: "JSON",   
		...
		function updateElement(json, params, obj) {
			...
			$.each(json, function(index, jsonobj) {	
				...
				$.each(jsonobj.info, function(key,value) {
					...
					if (key == 'append') {
						if(value['photo']) obj.append("<img src="+photo+" >");						
						if(value['text'])  obj.append("<p>"+text+"</p>");
					}
					...
					if (key == 'src') {
						if(value['photo']) obj.attr("src", photo);						
						if(value['text'])  obj.append("src", text);
					}
